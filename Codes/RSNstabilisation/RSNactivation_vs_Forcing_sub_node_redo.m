...
for kk=1:NPARCELLS/2
    ...
    infocapacity=std(KoPper-mean(KoP));
    susceptibility=mean(KoPper-mean(KoP));
    FC_pert_av(:,:) =squeeze(nanmean(FC_pert,3));
    FC_unp_av(:,:) =squeeze(nanmean(FC_unp,3));
    



save(sprintf('Wtrials_forz_sub_RSNact_%03d.mat',kk),'infocapacity','susceptibility','NSUBSIM','FC_pert_av','FC_unp_av');
end